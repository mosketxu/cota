<div>
    @forelse($detalles as $detalle)
        ...
        <input type="hidden" name="facturacionconcepto_id" value={{ $detalle->id }}>
            ...
            <div class="flex space-x-1" wire:loading.class.delay="opacity-50">
                ...
                <div class="w-6/12 "><input type="text" name="concepto" value="{{ $detalle->concepto }}" class="w-full py-1 text-sm {{ $color }} font-thin text-gray-500 truncate border-0 rounded-md" /></div>
                ...
                <div class="w-2/12 "><input type="number" name="importe" step="any" value="{{ $detalle->importe }}" class="w-full py-1 {{ $color }} text-right text-sm font-thin text-gray-500 truncate border-0 rounded-md"/></div>
    ...
    <form action="{{route('facturacionconceptodetalle.store')}}" method="post">
        ...
        <div class="flex space-x-1" wire:loading.class.delay="opacity-50">
            ...
            <div class="w-6/12 {{ $color }}"><input type="text" name="concepto" value="{{ old('concepto') }}" class="w-full py-1 text-sm font-thin text-gray-500 truncate border-0 rounded-md" /></div>
            ...
            <div class="w-2/12 {{ $color }}"><input type="number" name="importe" step="any" value="{{ old('importe','0') }}" class="w-full py-1 text-sm font-thin text-right text-gray-500 truncate border-0 rounded-md"/></div>
